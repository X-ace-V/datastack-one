...
Status: **proposed — awaiting confirmation** · supersedes the v1 wizard PRD
...
A local web app where a user opens a **chat session**, connects data (a CSV or a live
Postgres), and **asks in plain English** — *"profile this,"* *"which branch has the most
...
sessions** for different work and switch between them.
...
- **Sessions** — create / list / switch / rename / delete; each is its own OpenCode session;
  messages + metadata persisted so a session reopens with its history.
- **Chat** — NL input; streamed assistant text, reasoning, and **tool-call cards** (name +
  args + status + result); cancel a turn; per-session model.
...
- **Data sources** — per session: **CSV upload** and **registered Postgres connections**.
...
- **Agent tools** (the agent calls these conversationally):
  - read-only: `list_sources` (by name + schema), `profile_source`, `run_query` (SELECT over
    DuckDB + attached Postgres). Connecting a database is a **settings action**, not an agent
    tool — the agent never handles credentials.
...
- **FR1 — Sessions.** `POST/GET/PATCH/DELETE` sessions; each maps to an OpenCode session;
  messages + metadata persisted (DuckDB `platform` schema). Sidebar lists them; switching
  loads history.
...
- **FR3 — Event stream.** `GET /api/events` (SSE) relays OpenCode `event.subscribe` —
...
- **FR4 — CSV source.** Upload a CSV into a session → registered as a source, loadable/
  queryable in DuckDB.
...
- **FR10 — Inline approval gate.** `land_parquet`, `load_warehouse`, `run_transform`,
  `publish_serving` are permission `ask`; each surfaces inline in the chat with the exact
...
- [ ] A user can **create multiple sessions** and switch between them, each keeping its own
      history.
- [ ] From a session, **upload a CSV, ask a question in plain English, and get a correct
      table back** in the panel (`run_query`) — no buttons per step.
- [ ] **Connect a local Postgres** and query its tables (and join a CSV to a PG table) via NL.
- [ ] Ask the agent to **build and publish a daily branch report**; it profiles, writes SQL,
      **pauses inline for approval on each write**, runs it, and exposes a REST endpoint —
      all in conversation.
- [ ] **100% of write tools required an inline approval** before executing (verified from the
      lineage/audit trail).
- [ ] Tool calls **stream live** in the chat (reasoning + tool cards with status).
- [ ] The whole flow **completes on the free `opencode/big-pickle`** at least once (upgrading
...
as a plugin, `event.subscribe`→SSE, `permission.asked` bridge) and
